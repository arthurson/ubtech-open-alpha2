...
const I18N = {
  ...
  vosk_dl_supported_heading: { zh: "✓ 已支持（可對答）", en: "✓ Supported (dialogue-ready)" },
  vosk_dl_unsupported_heading: { zh: "未支持（只聽寫）", en: "Unsupported (transcribe only)" },
  ...
  actions_pack_hint:      { zh: "更新包有 202 個動作，下載並自動解壓到 sdcard（舊 /sdcard/actions 會改名 /sdcard/actions-backup）。完成後去動作頁重新載入列表。",
                             en: "The update pack contains 202 actions; downloads and auto-extracts to sdcard (old /sdcard/actions is renamed to /sdcard/actions-backup). Reload the action list in the Actions tab when done." },
  ...
  res_dl_hint: { zh: "一次只可下載一樣（Vosk 模型、動作包、Google TTS 共用通道）。Vosk 落完自動載入，語音頁即多一個按鍵；動作包落完去動作頁重新載入列表；Google TTS 落完自動開系統安裝器。",
                  en: "One download at a time (Vosk models, the actions pack and Google TTS share one slot). Vosk auto-loads when done, a new button appears in the Speech tab; reload the list in the Actions tab for the pack; Google TTS opens the system installer when done." },
  res_dl_show_unsupported: { zh: "顯示未支持模型 ▸", en: "Show unsupported models ▸" },
  res_dl_hide_unsupported: { zh: "▾ 收起未支持模型", en: "▾ Hide unsupported models" },
  // -- 資源下載卡 Google TTS 區（見 ApkDownloadController，淨下載去 sdcard） --
  apk_dl_heading: { zh: "📱 Google TTS 下載", en: "📱 Google TTS Download" },
  apk_dl_hint: { zh: "下載指定版本 Google TTS，落完自動開系統安裝器（經 scrcpy mouse 撳確認）。同卡其他下載共用通道，一次一樣。",
                  en: "Download the pinned Google TTS; the system installer opens when done (confirm with the scrcpy mouse). Shares the one download slot." },
  apk_dl_download_btn: { zh: "⬇ 下載 Google TTS", en: "⬇ Download Google TTS" },
  apk_dl_done: { zh: "✅ 已下載，已開系統安裝器（經 scrcpy 撳確認）", en: "✅ Downloaded, installer opened (confirm via scrcpy)" },
  ...
  vosk_hallu_hint:       { zh: "逐個開關做 AB 測試。grammar 切換聽緊會停完重開；前端兩項只存本瀏覽器。",
                            en: "Toggle each layer for A/B testing. Switching grammar restarts listening if active; frontend two are this-browser only." },
  vosk_hallu_confgate:   { zh: "認可度閘 (conf)", en: "Confidence gate (conf)" },
  vosk_hallu_conthr:     { zh: "閾值 0–1：", en: "Threshold 0–1:" },
  vosk_hallu_thr_apply:  { zh: "套用", en: "Apply" },
  vosk_hallu_dedup:      { zh: "4 秒去重", en: "4s dedup" },
  vosk_hallu_grammar:    { zh: "限定文法", en: "Constrained grammar" },
  vosk_hallu_unk:        { zh: "[unk] 清理", en: "[unk] cleanup" },
  vosk_hallu_ttspause:   { zh: "TTS 時停 decode", en: "Pause decode during TTS" },
  vosk_hallu_resumedelay: { zh: "播完 800ms 先 resume", en: "800ms resume delay" },
  vosk_hallu_frontquiet: { zh: "前端 2.5s 靜音窗（本機）", en: "Frontend 2.5s quiet window (local)" },
  vosk_hallu_frontsingle: { zh: "前端單字掉棄（本機）", en: "Frontend single-char drop (local)" },
