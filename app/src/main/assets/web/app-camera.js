...
async function startCameraLive() {
  ...
  await lynxApi("camera/resolution", { w: w, h: h });
...
async function takePhoto() {
  ...
  try {
    const json = await lynxApi("camera/snapshot");
    if (!json.ok) {
...
function flashCaptureLed() {
  const headBrightness = document.getElementById("lynxHeadBrightness").value;
  lynxApi("led/head/flash", { p0: 7, p1: headBrightness, p2: 100, p3: 500 });
  lynxApi("led/eye/flash", { p0: 7, p1: 9, p2: 100, p3: 500 });
}
...
function setupCrosshairIfNeeded() {
  ...
  function sendServoForAxis(nx, ny) {
    ...
    //
    // Uses the same move-time setting as the Servo tab (lynxServoTime()),
...
function connectCameraStream() {
  ...
  if (!img) {
    ...
    img.onload = function () {
      ...
      // comment above).
      lynxApi("camera/info", {}).then(function (resp) {
