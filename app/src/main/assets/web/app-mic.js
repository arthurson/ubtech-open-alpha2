// client logic (app-mic.js)
// 內容: 聽機械人麥克風 (WAV chunk 串流播放)、walkie-talkie (已永久停用)、downsample、相機全螢幕。
...
// testToneBtn/audioDiagBtn 呢兩個 id) —— 應該係之前改動連帶清埋
...
async function playTestTone() {
  ...
  try {
    const resp = await lynxApi("audio/testtone", {});
...
async function runAudioDiagnose() {
  ...
  try {
    const resp = await lynxApi("audio/diagnose", {});
...
async function startTalkDisabled_unused() {
  ...
  try {
    await lynxApi("audio/play/start", {});
