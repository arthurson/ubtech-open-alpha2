// client logic (app-lynx.js)
// 內容: Lynx (3.0.0.2) backend 全部功能 - 狀態/動作/servo/馬達/語音/LED。
// 全部檔案共用 window/global scope (冇用 ES module), 載入順序由 index.html 嘅
// <script src="..."> 順序決定 - 詳見 index.html 頭嗰段 comment。

// ---------------- Lynx (3.0.0.2) backend ----------------
// Async results (action list, motor list, ASR, ...) don't come back in the triggering
// HTTP response - they arrive as "lynx_*" WebSocket events (see appendLog()).
...
// -- Action --
// Reuses ACTION_CATEGORIES/ACTION_CATEGORY_MAP/categoryOf() (same 5 categories,
// same hardware) - only the underlying action list/state and API prefix are separate.
// 跟返 tab-actions 一樣, 除咗 5 大分類 (ACTION_CATEGORIES) 之外, 入面再有
...
// 嘅 javadoc: 呢份分類表係跟機身 action id 嚟, 可以直接共用)。
let lynxAllActions = [];
...
function lynxLoadActionList() {
  ...
  // action_classification.json 已經俾 Lynx 個 tab 用緊, loadActionClassification()
...
/** Same hardware (user-confirmed identical calibration), so this mirrors
 *  buildServoGridInto() exactly - same SERVO_GROUPS/SERVO_NAMES/SERVO_CALIBRATION,
 *  just lynxApi("motor/move_absolute"). Passes the extra readPrefix arg so each row
 *  gets its own hardware-readout span, filled in by lynxReadAllServoAngles() below. */
...
// -- LED --
// Lynx's ILedInterface is its own AIDL surface with separate methods
// (turnOnHead/Flash/Marquee/Breath etc.) with unlabelled int params (p0..p3).
