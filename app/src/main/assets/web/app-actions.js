// client logic (app-actions.js)
// 內容: 動作清單/分類共用邏輯 (buildActionSubSubTabsShared/renderActionChips)。
// 全部檔案共用 window/global scope (冇用 ES module), 載入順序由 index.html 嘅
// <script src="..."> 順序決定 - 詳見 index.html 頭嗰段 comment。

// ---------------- Actions ----------------

// Categories as returned by the robot's own action list.
...
function loadActionList() {
  ...
  return Promise.all([lynxApi("action/list"), loadActionClassification()]).then(function (results) {
 ...
 *  原本淨係 Alpha2 (buildActionSubSubTabs()) 有, 依家 Lynx (buildLynxActionSubSubTabs(),
 *  見 app-lynx.js) 都用返呢個, 跟 renderActionChips() 一樣抽出共用部分, 淨係
 *  「邊個 bar element」、「邊個 action 陣列」、「邊個 main category」、「目前揀咗邊個
 *  子分類 (+點樣寫返去)」由 caller 決定。action_classification.json 嘅 key 係機身
 *  action 服務嘅 id (Alpha2/Lynx 兩邊 ActionInfo.getId() 都係同一份, 見
 *  app-lynx.js「-- Action --」段頂嘅 comment: 同一部機、同一套 action, 淨係 AIDL
 *  surface 唔同), 所以呢份分類表兩邊可以直接共用, 唔使複製多一份。只有
 ...
 *  @param actions       完整動作陣列 (allActions 或 lynxAllActions)
 *  @param mainCategory  目前揀咗嘅大分類 key (activeActionCategory / activeLynxActionCategory)
...
/** 建立動作 chip 清單嘅共用邏輯 - renderActionList()/lynxRenderActionList() 之前
 *  渲染 chip 嗰部分 (innerHTML/forEach/建 chip/掛 onclick) 幾乎一樣, 淨係「過濾
 *  邏輯」、「顯示名點揀」、「撳落去入邊個 input/call 邊個播放 function」唔同 -
 *  將呢部分抽出嚟, 過濾/顯示/播放呢三樣留返俾兩個 caller 自己決定。
