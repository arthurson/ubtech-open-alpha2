 ...
 *
 * UNVERIFIED PROVENANCE: this class does NOT wrap any AIDL/alpha2serverlib method -
 * there is no such thing as "mouth LED" in the AIDL interface this project otherwise
 * uses. It wraps com.ubtechinc.mic5.LedControl, a native JNI class (backed by
 * libhead_led.so) found in a separate demo APK (alpha2demo).
 *
 * (Note: an earlier version of this comment compared this class against
 * header_ledSetHead5Mic/header_ledSetEye5Mic on Alpha2RobotApi. Those two methods have
 * since been removed - re-decompiling the target APK's IAlpha2SerialPortService$Stub
 * shows they were never real AIDL transactions in this build. The comparison below to
 * "dual"=3 is kept only as a description of a value from that removed API, for anyone
 * cross-referencing an older build that did have it.)
 *
 ...
 * DIFFERENT path from the AIDL-bound alpha2services process used elsewhere in this SDK
 * (e.g. IAlpha2SerialPortService's sendCommand). Whether the two paths can safely be
 * used concurrently (e.g. do they contend for the same underlying serial/LED device)
 * has NOT been verified against real hardware. If head/eye LEDs stop responding
 * correctly after a mouth LED call, that's this contention risk manifesting.
 */
