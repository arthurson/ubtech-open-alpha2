...
public class AudioPlaybackController {
    ...
    // Jitter-buffer cap: bytes/sec at 16kHz mono 16-bit = 32000 (doubled back up from
    // 16000 now that SAMPLE_RATE_HZ is back to 16000 above - this formula must track
    // that change, or the cap's actual duration would silently halve instead of
    // staying at the intended ~600ms). Each browser chunk (ScriptProcessorNode
    // bufferSize=4096 at the browser's native rate, downsampled to 16kHz - see app-mic.js)
    ...
    private static final int JITTER_BUFFER_CAP_BYTES = 32000 * 600 / 1000; // ~600ms = 19200 bytes
    ...
    /** Queues raw PCM (16kHz mono 16-bit, matching SAMPLE_RATE_HZ/CHANNEL_CONFIG/
