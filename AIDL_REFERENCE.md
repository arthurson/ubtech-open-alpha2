...
| 7 | `setWakeState` | `boolean onWake` | ⚠️ **唔係「開始聆聽」**！呢個淨係將 mic 擁有權喺 app 同 alpha2services 之間轉手（`true` = 釋放俾機械人，`false` = app 攞返）。內部行為係將 ASR 引擎設做 idle 同 destroy 現有 recognition session，**唔會主動觸發辨識** |
...
`speech_SetMIC(true)`（對應 `setWakeState(true)`）**唔會觸發辨識**——佢淨係釋放 mic 擁有權，之後要靠機械人自己嘅硬件 wake-word 偵測（`MicArray wakeup` log）先會真正開始聆聽，而且呢個偵測時機唔受 app 控制。用 `startSpeechNoWakeup` 可以更早將 engine 撥入 wake-word-receptive 狀態，但都唔係「即刻聽到」（見上面 #19 嘅修正）。
