...
| 2 | `onSpeech` | `listener, String text` | ⚠️ **2026-08 logcat 覆核推翻舊結論**：之前以為「文字直接注入去語意理解層，唔經真正麥克風／ASR」，但實測（`logcat_2026-08-01_13-19-46.txt`，經 `Alpha2RobotApi.speech_startRecognized()` 呼叫）顯示 `SpeechServiceImpl.onSpeech()` 喺呢部機嘅實際行為係**直接將傳入嘅 `text` 用 TTS 讀返出嚟**——logcat 見到 call 完即刻 `SpeechManager` 印 `ttsIsSpeaking:false language: text = stand up`，跟住 `start tts play` → `IflytekTTS` 真係讀出「stand up」呢句話，全程冇任何辨識/語意理解相關嘅 callback 或 event 觸發。即係話呢個 method 喺呢個 firmware 版本上根本唔係「假裝聽到某句嘢」嘅辨識模擬入口，而係一個側門 TTS：**傳入嘅文字會被讀出嚟，而唔係被拎去辨識**。SDK 標呢個 method `@Deprecated` 好可能就係因為呢個名不副實嘅行為。想測試「辨識」相關功能（ASR/NLU/grammar），呢個 method 唔可靠，應該用 `initSpeechGrammar`/`startSpeechGrammar`（第 9、10 項，要切去 iFlytek binding）或者 `onTextUnderstand`（第 8 項，已知冧咗，見下） |
| 3 | `onStopSpeech` | `listener` | 停止上面第 2 項 `onSpeech` 觸發嘅嘢（實測係停止緊 TTS 讀出，唔係停止「模擬輸入辨識」——見第 2 項 2026-08 覆核） |
