...
| `come.ubt.alpha2.gesture` | `"getstureDirection"`（拼字是原廠錯字，`Integer` 非 `String`）| 頭頂 `+/-` pad 按鍵事件（非視覺手勢）—— `NativeClassJni` 輪詢 `/dev/input/event*` `rk29-keypad` 經 `com.example.t01.nativeClass.onNativeCallback(I)` 轉發。實測值為 `Integer` `raw = (eventCode << 8) | 0x01`，低位恆 `0x01`，高位 `eventCode` 才是事件：`0x5a(90) "-" 按住`→`23041`、`0x5b(91) "-" 放開`→`23297`、`0x5c(92) "+" 按住`→`23553`、`0x5d(93) "+" 放開`→`23809`、`0x5e(94) 雙鍵齊按`→`24065`、`0x5f(95) 雙鍵齊放`→`24321`（`MainActivity.java:850` 已實測對照）。用 `getStringExtra()` 會拋 `ClassCastException`，須用 `Bundle.get()`/`getIntExtra()` 讀 |
