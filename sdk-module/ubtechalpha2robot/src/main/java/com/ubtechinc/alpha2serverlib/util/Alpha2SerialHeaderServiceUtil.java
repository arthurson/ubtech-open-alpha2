...
public class Alpha2SerialHeaderServiceUtil implements ServiceConnection {
   ...
   /**
    * 2026-08 新增: 加回這個 overload 讓 caller 可以指定實際想等多久, 而不是永遠
    * 硬吃 ~3s。之前 Alpha2RobotApi.waitHeaderReady(long timeoutMs) 的 timeoutMs
    * 參數其實沒有傳到這一層, 靜靜地被忽略, 讓傳入的值完全沒有作用。
    *
    * @param timeoutMs 最多等多少 ms (負數當 0 處理)。在主 thread 呼叫照樣立刻
    *                  return (見上面 no-arg 版本的原因)。
    */
