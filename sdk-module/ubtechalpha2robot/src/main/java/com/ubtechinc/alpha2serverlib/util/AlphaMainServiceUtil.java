...
public class AlphaMainServiceUtil {
   ...
   // 2026-08 修正: startService()/stopService() 之前用緊 implicit Intent (淨係
   // Intent(ACTION), 冇 setPackage()) - targetSdkVersion 22 底下, Android 5.0+
   // (API 21+) 對 startService() 已經會拋 IllegalArgumentException("Service Intent
   // must be explicit"), 同呢個 project 其他六個 *ServiceUtil class (全部
   // bindService() 都有 setPackage(SERVICE_PACKAGE)) 應該一致, 但呢個 class 因為
   // 用嘅係 startService()/stopService() 唔係 bindService(), 之前執漏咗冇加。
