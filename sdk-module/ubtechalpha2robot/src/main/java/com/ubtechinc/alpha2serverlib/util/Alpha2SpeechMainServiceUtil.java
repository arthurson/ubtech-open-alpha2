...
public class Alpha2SpeechMainServiceUtil implements ServiceConnection {
   ...
   private IAlphaEnglishUnderstandListener mEnglishUnderstandListener;
   private IAlphaEnglishOfflineUnderstandListener mEnglishOfflineUnderstandListener;
   private IReplaySpeechCallback mReplaySpeechListener;
    ...
    * 機身系統進程 (com.ubtechinc.alpha2services) 裡面的 TTS session 就會啞
    * (onPlay/onPlayHigh 都還會回 API_ERROR_SUCCEED, 但完全沒有後續的
    * SpeechServiceImpl/IflytekTTS/onTTsStart log, 也就是說這個 binder call 打到
    * 一個已經壞死的 session, 不會拋 RemoteException), 一直要重開機才能回到正常。
    * <p>
    * 這個方法叫機身韌體將 speech engine 拉回 idle 狀態, 理論上可以在不用重開機
    * 的情況下清掉壞掉的 session。要留意這個 call 應該找回 generic alias
    * binding (mSpeechServiceUtil, 一開機用 ALPHA_SPEECH_MAIN_SERVER bind 那個)
    * 來做, 不是那個已經死掉的 direct-engine binding, 因為後者本身可能已經連這個
    * call 都打不到。還沒在真機驗證過這個方法是否真的能解決問題, 純粹跟 AIDL 方法名
    * 和用途做的合理推測。
    */
    ...
    * (ISpeechInterface transaction #15). Unlike onTextUnderstand, this is a
    * standing registration, not a per-call listener - the AIDL method takes only
    * the listener, no text, so results presumably arrive off of whatever ASR/NLU
    * pipeline the robot runs internally. Unverified against real hardware.
    */
   public void onEnglishUnderstand(IAlphaEnglishUnderstandListener listener) {
      this.mEnglishUnderstandListener = listener;
      if (this.mService == null) {
         return;
      }
      try {
         this.mService.onEnglishUnderstand(new IAlphaEnglishUnderstandListener.Stub() {
            @Override
            public void onAlpha2EnglishUnderstandResult(String strResult) throws RemoteException {
               if (Alpha2SpeechMainServiceUtil.this.mEnglishUnderstandListener != null) {
                  Alpha2SpeechMainServiceUtil.this.mEnglishUnderstandListener.onAlpha2EnglishUnderstandResult(strResult);
               }
            }
         });
      } catch (RemoteException e) {
    ...
    * (ISpeechInterface transaction #16). See onEnglishUnderstand's javadoc - same
    * standing-registration shape, offline engine counterpart.
    */
   public void setEnglishOfflineListener(IAlphaEnglishOfflineUnderstandListener listener) {
      this.mEnglishOfflineUnderstandListener = listener;
      if (this.mService == null) {
         return;
      }
      try {
         this.mService.setEnglishOfflineListener(new IAlphaEnglishOfflineUnderstandListener.Stub() {
            @Override
   ...
   /**
    * Registers a listener to receive replayed ASR history records (ISpeechInterface
    * transaction #22, see IReplaySpeechCallback/ASRRecord). What triggers the robot
    * to start sending records - and whether it sends an existing backlog or only
    * new recognitions going forward - is unverified against real hardware; this
    * only wires the registration through.
    */
   public void registerReplayContentListener(IReplaySpeechCallback listener) {
      this.mReplaySpeechListener = listener;
      if (this.mService == null) {
         return;
      }
      try {
         this.mService.registerReplayContentListener(new IReplaySpeechCallback.Stub() {
            @Override
            public void onRelpayContent(ASRRecord record) throws RemoteException {
               if (Alpha2SpeechMainServiceUtil.this.mReplaySpeechListener != null) {
                  Alpha2SpeechMainServiceUtil.this.mReplaySpeechListener.onRelpayContent(record);
               }
            }
         });
      } catch (RemoteException e) {
   ...
   public void onServiceConnected(ComponentName name, IBinder service) {
      ...
      // registerSpeechCallBackListener must complete BEFORE initOver() fires.
      // initOver() is what tells the app "speech is ready" (MainActivity flips
      // speechReady = true and emits the speech_ready event), so any caller
      // reacting to that signal needs the listener to already be registered on
      // the robot side - otherwise a fast-following speech/tts or ASR call can
