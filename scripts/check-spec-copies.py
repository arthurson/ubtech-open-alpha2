#!/usr/bin/env python3
"""
Web 側 spec copy 一致性檢查 (openapi/ 係單一真相源)。
...
嗰幾份係 `openapi/` 嘅 copy (另加 `well-known/` 無點版，留低以防出面有用)，
之前全靠人手 sync——試過 asyncapi copy 留低已死嘅 binder 事件
(asr_result/speech_ready) 而源頭已清，無人發現。

規則：
- yml copy 必須同源頭 byte-identical (手改 copy 即 drift)。
- json copy 必須同源頭 parse 之後 semantic equal (格式轉換，逐 byte 比唔到)。

用法:
  python scripts/check-spec-copies.py         # 检查，drift 即 exit 1 (啱 CI 用)
  python scripts/check-spec-copies.py --fix   # 把源頭 byte-copy 蓋落 drift 咗嘅
                                              # yml copy (json 唔識得原 producer
                                              # 格式設定，唔碰，只報告)
"""
import difflib
import json
...
def check_json(src: pathlib.Path, dst: pathlib.Path) -> bool:
    ...
    if want == got:
        ...
        return True
    print(f"DIFF {dst.relative_to(ROOT)} (semantic，同源頭唔等)")
...
def main() -> int:
    ...
    if fix and bad_yml:
        ...
        bad_yml = []
    if fix and bad_json:
        print("json drift 唔識得原 producer 格式設定，唔自動修，請人手處理")
